...
class OGBNDataset(object):
    ...
    def __init__(self, dataset_name='ogbn-proteins'):
        ...
        self.dataset = OGB(data_name=self.dataset_name)
        self.splitted_idx = self.dataset.get_split()
        self.whole_graph = self.dataset.get_data()
        self.length = 1

        self.train_idx, self.valid_idx, self.test_idx = self.splitted_idx["train"], self.splitted_idx["valid"], self.splitted_idx["test"]
        self.num_tasks = self.whole_graph.y.size(1)
    ...
    def extract_node_features(self, aggr='add'):
        ...
        file_path = 'output/init_node_features_{}.pt'.format(aggr)

        if not os.path.isfile(file_path):
            if aggr in ['add', 'mean', 'max']:
                node_features = scatter(self.edge_attr, self.edge_index[0], dim=0, dim_size=self.total_no_of_nodes, reduce=aggr)
            else:
    ...
    def construct_adj(self):
        adj = sp.csr_matrix((np.ones(self.total_no_of_edges, dtype=np.uint8), (self.edge_index_array[:, 0], self.edge_index_array[:, 1])), shape=(self.total_no_of_nodes, self.total_no_of_nodes))
        return adj

    def edge_features_index(self):
        file_name = 'output/edge_features_index_v2.pkl'
